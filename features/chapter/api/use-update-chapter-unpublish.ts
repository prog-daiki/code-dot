...
export const useUpdateChapterUnPublish = (courseId: string, chapterId: string) => {
  const queryClient = useQueryClient();
  const mutation = useMutation<ResponseType, Error>({
    mutationFn: async () => {
      const response = await client.api.courses[":course_id"].chapters[":chapter_id"].unpublish.$put({
        param: { course_id: courseId, chapter_id: chapterId },
      });
      if (!response.ok) {
        throw new Error("チャプターの非公開に失敗しました");
      }
